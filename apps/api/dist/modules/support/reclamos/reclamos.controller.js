"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReclamosController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const reclamos_service_1 = require("./reclamos.service");
const create_reclamo_dto_1 = require("./dto/create-reclamo.dto");
const query_reclamos_dto_1 = require("./dto/query-reclamos.dto");
const update_reclamo_dto_1 = require("./dto/update-reclamo.dto");
const create_archivo_reclamo_dto_1 = require("./dto/create-archivo-reclamo.dto");
const current_user_decorator_1 = require("../../../common/decorators/current-user.decorator");
let ReclamosController = class ReclamosController {
    constructor(reclamosService) {
        this.reclamosService = reclamosService;
    }
    crear(user, dto) {
        return this.reclamosService.crear(user.empresaId, dto);
    }
    listar(user, query) {
        return this.reclamosService.listar(user.empresaId, query);
    }
    obtener(user, id) {
        return this.reclamosService.obtener(user.empresaId, id);
    }
    actualizar(user, id, dto) {
        return this.reclamosService.actualizar(user.empresaId, id, dto);
    }
    agregarArchivo(user, id, dto) {
        return this.reclamosService.agregarArchivo(user.empresaId, id, dto);
    }
};
exports.ReclamosController = ReclamosController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, create_reclamo_dto_1.CreateReclamoDto]),
    __metadata("design:returntype", void 0)
], ReclamosController.prototype, "crear", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, query_reclamos_dto_1.QueryReclamosDto]),
    __metadata("design:returntype", void 0)
], ReclamosController.prototype, "listar", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], ReclamosController.prototype, "obtener", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, update_reclamo_dto_1.UpdateReclamoDto]),
    __metadata("design:returntype", void 0)
], ReclamosController.prototype, "actualizar", null);
__decorate([
    (0, common_1.Post)(':id/archivos'),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, create_archivo_reclamo_dto_1.CreateArchivoReclamoDto]),
    __metadata("design:returntype", void 0)
], ReclamosController.prototype, "agregarArchivo", null);
exports.ReclamosController = ReclamosController = __decorate([
    (0, swagger_1.ApiTags)('reclamos'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)('reclamos'),
    __metadata("design:paramtypes", [reclamos_service_1.ReclamosService])
], ReclamosController);
//# sourceMappingURL=reclamos.controller.js.map